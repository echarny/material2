 ...
 */
var ClassExportDoc = /** @class */ (function (_super) {
    __extends(ClassExportDoc, _super);
    function ClassExportDoc(moduleDoc, symbol, aliasSymbol) {
        var _this = _super.call(this, moduleDoc, symbol, symbol.valueDeclaration, aliasSymbol) || this;
        _this.docType = 'class';
        _this.statics = [];
        _this.isAbstract = _this.declaration.modifiers && _this.declaration.modifiers.some(function (modifier) { return modifier.kind === typescript_1.SyntaxKind.AbstractKeyword; });
        if (symbol.exports) {
            _this.statics = _this.getMemberDocs(symbol.exports, moduleDoc.hidePrivateMembers);
        }
        if (symbol.members) {
            // Get the constructor
            var constructorSymbol = symbol.members.get(typescript_1.InternalSymbolName.Constructor);
            ...
            _this.members = _this.getMemberDocs(symbol.members, moduleDoc.hidePrivateMembers);
        }
        return _this;
    }
    ClassExportDoc.prototype.getConstructorDoc = function (constructorSymbol) {
        ...
        constructorSymbol.getDeclarations().forEach(function (declaration) {
            if (declaration.body) {
                ...
                constructorDoc = new MethodMemberDoc_1.MethodMemberDoc(_this, constructorSymbol, declaration, overloads);
            }
            else {
                // This is an overload signature of the method
                overloads.push(new MethodMemberDoc_1.MethodMemberDoc(_this, constructorSymbol, declaration, overloads));
            }
        });
