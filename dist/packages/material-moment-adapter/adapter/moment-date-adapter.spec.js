...
describe('MomentDateAdapter', function () {
    ...
    it('should get year', function () {
        expect(adapter.getYear(moment([2017, core_2.JAN, 1]))).toBe(2017);
    });
    it('should get month', function () {
        expect(adapter.getMonth(moment([2017, core_2.JAN, 1]))).toBe(0);
    });
    it('should get date', function () {
        expect(adapter.getDate(moment([2017, core_2.JAN, 1]))).toBe(1);
    });
    it('should get day of week', function () {
        expect(adapter.getDayOfWeek(moment([2017, core_2.JAN, 1]))).toBe(0);
    });
    ...
    it('should get year name', function () {
        expect(adapter.getYearName(moment([2017, core_2.JAN, 1]))).toBe('2017');
    });
    ...
    it('should create Moment date', function () {
        expect(adapter.createDate(2017, core_2.JAN, 1).format())
            .toEqual(moment([2017, core_2.JAN, 1]).format());
    });
    it('should not create Moment date with month over/under-flow', function () {
        expect(function () { return adapter.createDate(2017, core_2.DEC + 1, 1); }).toThrow();
        expect(function () { return adapter.createDate(2017, core_2.JAN - 1, 1); }).toThrow();
    });
    it('should not create Moment date with date over/under-flow', function () {
        expect(function () { return adapter.createDate(2017, core_2.JAN, 32); }).toThrow();
        expect(function () { return adapter.createDate(2017, core_2.JAN, 0); }).toThrow();
    });
    it('should create Moment date with low year number', function () {
        expect(adapter.createDate(-1, core_2.JAN, 1).year()).toBe(-1);
        expect(adapter.createDate(0, core_2.JAN, 1).year()).toBe(0);
        expect(adapter.createDate(50, core_2.JAN, 1).year()).toBe(50);
        expect(adapter.createDate(99, core_2.JAN, 1).year()).toBe(99);
        expect(adapter.createDate(100, core_2.JAN, 1).year()).toBe(100);
    });
    it('should not create Moment date in utc format', function () {
        expect(adapter.createDate(2017, core_2.JAN, 5).isUTC()).toEqual(false);
    });
    ...
    it('should parse string according to given format', function () {
        expect(adapter.parse('1/2/2017', 'MM/DD/YYYY').format())
            .toEqual(moment([2017, core_2.JAN, 2]).format());
        expect(adapter.parse('1/2/2017', 'DD/MM/YYYY').format())
            .toEqual(moment([2017, core_2.FEB, 1]).format());
    });
    ...
    it('should format date according to given format', function () {
        expect(adapter.format(moment([2017, core_2.JAN, 2]), 'MM/DD/YYYY')).toEqual('01/02/2017');
        expect(adapter.format(moment([2017, core_2.JAN, 2]), 'DD/MM/YYYY')).toEqual('02/01/2017');
    });
    it('should format with a different locale', function () {
        expect(adapter.format(moment([2017, core_2.JAN, 2]), 'll')).toEqual('Jan 2, 2017');
        adapter.setLocale('ja-JP');
        expect(adapter.format(moment([2017, core_2.JAN, 2]), 'll')).toEqual('2017年1月2日');
    });
    ...
    it('should add years', function () {
        expect(adapter.addCalendarYears(moment([2017, core_2.JAN, 1]), 1).format())
            .toEqual(moment([2018, core_2.JAN, 1]).format());
        expect(adapter.addCalendarYears(moment([2017, core_2.JAN, 1]), -1).format())
            .toEqual(moment([2016, core_2.JAN, 1]).format());
    });
    it('should respect leap years when adding years', function () {
        expect(adapter.addCalendarYears(moment([2016, core_2.FEB, 29]), 1).format())
            .toEqual(moment([2017, core_2.FEB, 28]).format());
        expect(adapter.addCalendarYears(moment([2016, core_2.FEB, 29]), -1).format())
            .toEqual(moment([2015, core_2.FEB, 28]).format());
    });
    it('should add months', function () {
        expect(adapter.addCalendarMonths(moment([2017, core_2.JAN, 1]), 1).format())
            .toEqual(moment([2017, core_2.FEB, 1]).format());
        expect(adapter.addCalendarMonths(moment([2017, core_2.JAN, 1]), -1).format())
            .toEqual(moment([2016, core_2.DEC, 1]).format());
    });
    it('should respect month length differences when adding months', function () {
        expect(adapter.addCalendarMonths(moment([2017, core_2.JAN, 31]), 1).format())
            .toEqual(moment([2017, core_2.FEB, 28]).format());
        expect(adapter.addCalendarMonths(moment([2017, core_2.MAR, 31]), -1).format())
            .toEqual(moment([2017, core_2.FEB, 28]).format());
    });
    it('should add days', function () {
        expect(adapter.addCalendarDays(moment([2017, core_2.JAN, 1]), 1).format())
            .toEqual(moment([2017, core_2.JAN, 2]).format());
        expect(adapter.addCalendarDays(moment([2017, core_2.JAN, 1]), -1).format())
            .toEqual(moment([2016, core_2.DEC, 31]).format());
    });
    ...
    it('should compare dates', function () {
        expect(adapter.compareDate(moment([2017, core_2.JAN, 1]), moment([2017, core_2.JAN, 2]))).toBeLessThan(0);
        expect(adapter.compareDate(moment([2017, core_2.JAN, 1]), moment([2017, core_2.FEB, 1]))).toBeLessThan(0);
        expect(adapter.compareDate(moment([2017, core_2.JAN, 1]), moment([2018, core_2.JAN, 1]))).toBeLessThan(0);
        expect(adapter.compareDate(moment([2017, core_2.JAN, 1]), moment([2017, core_2.JAN, 1]))).toBe(0);
        expect(adapter.compareDate(moment([2018, core_2.JAN, 1]), moment([2017, core_2.JAN, 1]))).toBeGreaterThan(0);
        expect(adapter.compareDate(moment([2017, core_2.FEB, 1]), moment([2017, core_2.JAN, 1]))).toBeGreaterThan(0);
        expect(adapter.compareDate(moment([2017, core_2.JAN, 2]), moment([2017, core_2.JAN, 1]))).toBeGreaterThan(0);
    });
    it('should clamp date at lower bound', function () {
        expect(adapter.clampDate(moment([2017, core_2.JAN, 1]), moment([2018, core_2.JAN, 1]), moment([2019, core_2.JAN, 1])))
            .toEqual(moment([2018, core_2.JAN, 1]));
    });
    it('should clamp date at upper bound', function () {
        expect(adapter.clampDate(moment([2020, core_2.JAN, 1]), moment([2018, core_2.JAN, 1]), moment([2019, core_2.JAN, 1])))
            .toEqual(moment([2019, core_2.JAN, 1]));
    });
    it('should clamp date already within bounds', function () {
        expect(adapter.clampDate(moment([2018, core_2.FEB, 1]), moment([2018, core_2.JAN, 1]), moment([2019, core_2.JAN, 1])))
            .toEqual(moment([2018, core_2.FEB, 1]));
    });
    ...
    it('returned Moments should have correct locale', function () {
        adapter.setLocale('ja-JP');
        expect(adapter.createDate(2017, core_2.JAN, 1).locale()).toBe('ja');
...
describe('MomentDateAdapter with LOCALE_ID override', function () {
    ...
    it('should take the default locale id from the LOCALE_ID injection token', function () {
        expect(adapter.format(moment([2017, core_2.JAN, 2]), 'll')).toEqual('2 janv. 2017');
    });
});
describe('MomentDateAdapter with MAT_MOMENT_DATE_ADAPTER_OPTIONS override', function () {
    ...
    describe('use UTC', function () {
        it('should create Moment date in UTC', function () {
            expect(adapter.createDate(2017, core_2.JAN, 5).isUtc()).toBe(true);
        });
