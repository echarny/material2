...
ItemSizeAverager = /** @class */ (function () {
     ...
     */
    function (range, size) {
        var /** @type {?} */ newTotalWeight = this._totalWeight + range.end - range.start;
        if (newTotalWeight) {
            var /** @type {?} */ newAverageItemSize = (size + this._averageItemSize * this._totalWeight) / newTotalWeight;
...
function ItemSizeAverager_tsickle_Closure_declarations() {
    /**
     * The total amount of weight behind the current average.
     * @type {?}
     */
...
AutoSizeVirtualScrollStrategy = /** @class */ (function () {
    ...
    function AutoSizeVirtualScrollStrategy(minBufferPx, maxBufferPx, averager) {
         ...
         */
        this.scrolledIndexChange = Observable.create(function () {
     ...
     */
    function () {
        var /** @type {?} */ viewport = /** @type {?} */ ((this._viewport));
        // The current scroll offset.
        var /** @type {?} */ scrollOffset = viewport.measureScrollOffset();
        // The delta between the current scroll offset and the previously recorded scroll offset.
        var /** @type {?} */ scrollDelta = scrollOffset - this._lastScrollOffset;
        // The magnitude of the scroll delta.
        var /** @type {?} */ scrollMagnitude = Math.abs(scrollDelta);
        // The currently rendered range.
        var /** @type {?} */ renderedRange = viewport.getRenderedRange();
        ...
        var /** @type {?} */ offsetCorrection = 0;
        if (scrollDelta < 0) {
            // The content offset we would expect based on the average item size.
            var /** @type {?} */ predictedOffset = renderedRange.start * this._averager.getAverageItemSize();
            ...
            var /** @type {?} */ offsetDifference = predictedOffset - this._lastRenderedContentOffset;
        ...
        var /** @type {?} */ startBuffer = this._lastScrollOffset - this._lastRenderedContentOffset;
        // The current amount of buffer past the end of the viewport.
        var /** @type {?} */ endBuffer = (this._lastRenderedContentOffset + this._lastRenderedContentSize) -
        ...
        var /** @type {?} */ underscan = scrollMagnitude + this._minBufferPx -
        ...
        if (underscan > 0) {
            ...
            if (scrollMagnitude >= viewport.getViewportSize()) {
                ...
                var /** @type {?} */ addItems = Math.max(0, Math.ceil((underscan - this._minBufferPx + this._maxBufferPx) /
                    this._averager.getAverageItemSize()));
                // The amount of filled space beyond what is necessary on the side the user is scrolling
                // away from.
                var /** @type {?} */ overscan = (scrollDelta < 0 ? endBuffer : startBuffer) - this._minBufferPx +
                    scrollMagnitude;
                // The number of currently rendered items to remove on the side the user is scrolling away
                // from. If removal has failed in recent cycles we are less aggressive in how much we try to
                // remove.
                var /** @type {?} */ unboundedRemoveItems = Math.floor(overscan / this._averager.getAverageItemSize() / (this._removalFailures + 1));
                var /** @type {?} */ removeItems = Math.min(renderedRange.end - renderedRange.start, Math.max(0, unboundedRemoveItems));
                // The new range we will tell the viewport to render. We first expand it to include the new
                // items we want rendered, we then contract the opposite side to remove items we no longer
                // want rendered.
                var /** @type {?} */ range = this._expandRange(renderedRange, scrollDelta < 0 ? addItems : 0, scrollDelta > 0 ? addItems : 0);
                if (scrollDelta < 0) {
                ...
                var /** @type {?} */ contentOffset = void 0;
                var /** @type {?} */ contentOffsetTo = void 0;
                if (scrollDelta < 0) {
                    var /** @type {?} */ removedSize = viewport.measureRangeSize({
                        start: range.end,
     ...
     * @return {?}
     */
    AutoSizeVirtualScrollStrategy.prototype._setScrollOffset = /**
     * Sets the scroll offset and renders the content we estimate should be shown at that point.
     * @param {?=} scrollOffset The offset to jump to. If not specified the scroll offset will not be
     *     changed, but the rendered content will be recalculated based on our estimate of what should
     *     be shown at the current scroll offset.
    ...
    function (scrollOffset) {
        ...
        this._lastScrollOffset = scrollOffset;
        this._removalFailures = 0;
        var /** @type {?} */ itemSize = this._averager.getAverageItemSize();
        var /** @type {?} */ firstVisibleIndex = Math.min(viewport.getDataLength() - 1, Math.floor(scrollOffset / itemSize));
        var /** @type {?} */ bufferSize = Math.ceil(this._maxBufferPx / itemSize);
        var /** @type {?} */ range = this._expandRange(this._getVisibleRangeForIndex(firstVisibleIndex), bufferSize, bufferSize);
        viewport.setRenderedRange(range);
    ...
    function (startIndex) {
        ...
        var /** @type {?} */ extra = range.end - viewport.getDataLength();
        if (extra > 0) {
    ...
    function (range, expandStart, expandEnd) {
        ...
        var /** @type {?} */ end = Math.min(viewport.getDataLength(), range.end + expandEnd);
    ...
    function (renderedContentSize) {
        var /** @type {?} */ viewport = /** @type {?} */ ((this._viewport));
        var /** @type {?} */ renderedRange = viewport.getRenderedRange();
        var /** @type {?} */ totalSize = renderedContentSize +
...
function AutoSizeVirtualScrollStrategy_tsickle_Closure_declarations() {
    /**
     * \@docs-private Implemented as part of VirtualScrollStrategy.
     * @type {?}
     */
...
function CdkAutoSizeVirtualScroll_tsickle_Closure_declarations() {
     ...
     */
    CdkAutoSizeVirtualScroll.ctorParameters;
